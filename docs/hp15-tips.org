...
Almost all scientific calculators support at last four types of regression:
...
These four regression types may be computed from simple summary statistics, and are thus suitable even for calculators with very limited RAM.
Unfortunately the HP-15C only supports linear regression out of the box.  So we must do a bit more work to compute the others.  Luckily, it's
not hard -- we just need to transform the data as we enter it, and transform answers we extract.
