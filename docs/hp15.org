...
My primary intent for this program is to provide a good example illustrating =STO_CPLX= & =RCL_CPLX= in a program, and thus the super simple implementation
...
- Exit Information
  ...
  - Exit Register State:
    ...
    - {{{reg( 9 )}}} $\Im(x_n)$
  - Non-normal Exits:
   - *Convergence failure:* In this case the program should be interrupted by the user.  The last guess will likely be stored in registers {{{reg( 8 )}}} & {{{reg( 9 )}}}, but this is not guaranteed!
...
- Operation
  - Run the program with the following inputs: 
    - {{{stk(Z)}}}: A matrix descriptor or number =0= (a =0= indicates the {{{mat(A)}}} matrix should be used)
...
This program, =RCL_MAT=, provides a fast way iterate over the elements of a real or complex matrix.
 ...
 - Operation
  - Run the program with the following inputs: 
    - {{{stk(X)}}}: A matrix descriptor or number =0= (a =0= indicates the {{{mat(RESULT)}}} matrix should be used)
...
- Registers Used:
  ...
  - {{{reg( I )}}} Matrix descripter
- Matrices:
  - Matrix on stack level {{{stk(X)}}} when the program starts, or {{{mat(RESULT)}}} if {{{stk(X)}}} was zero.
...
- Program end conditions
  ...
  - Matrix {{{mat(C)}}} will be selected as the {{{mat(RESULT)}}} matrix.
...
- Matrices:
  ...
  - {{{mat( B )}}} Input
  - {{{mat( C )}}} The solution
  - {{{mat( E )}}} LU factorization when flag {{{flg( 1 )}}} is clear
...
This program computes the determinant of a real matrix or the magnitude of the determinant for a complex matrix.  The interactive use case is to determine if
a matrix is singular.  The non-interactive use case is {{{lbl(6)}}} -- =LIN_SLV= which uses this as a subroutine to 1) detect & transform complex matrices, 2)
compute the determinant or determinant magnitude, and 3) compute an LU factorization for the solution.
...
- Program start conditions
  - The $A$ matrix is assumed to be in {{{mat(A)}}}.  
    - It may be an $n\times n$ real matrix, or an  $n\times 2n$ complex matrix in *=C=* format.
...
- Program end conditions
  ...
  - If flag {{{flg( 1 )}}} is clear
    - {{{mat(A)}}} is left unchanged!
    - {{{mat(RESULT)}}} matrix set as {{{mat(E)}}}
  - If flag {{{flg( 1 )}}} is set
    - {{{mat(RESULT)}}} matrix set as {{{mat(A)}}}
  - $A$ is a real matrix
    - Flag {{{flg( 0 )}}} will be clear
...
This program transforms the matrices in {{{mat(A)}}} & {{{mat(B)}}} into normal form in place.  That is to say {{{mat(A)}}} is replaced with \( A^T A \) &
{{{mat(B)}}} is replaced with \( A^T B \).  I mostly use this to solve small, overdetermined systems.  This method is pretty poor practice from a numerical
analysis standpoint; however, it is simple and has generally worked for my applications.  Note that when the program completes, the {{{mat(A)}}} &
{{{mat(B)}}} matrices will be ready for {{{lbl(6)}}} -- =LIN_SLV=.

*** Usage
:PROPERTIES:
:CUSTOM_ID: matNrmUsage
:END:

- Program start conditions
  - The {{{mat(A)}}} & {{{mat(B)}}} matrices have been appropriately initialized
- Program end conditions
  - The previous contents of the stack are lost
  - The {{{mat(A)}}} matrix contains \( A^T A \)
  - The {{{mat(B)}}} matrix contains \( A^T B \)
  - The {{{mat(E)}}} matrix will be deallocated
  - {{{mat(RESULT)}}} will be set to {{{mat(E)}}}
...
*** Program Listing
:PROPERTIES:
:CUSTOM_ID: matNrmCode
:END:

#+BEGIN_CENTER
=MAT_NML= -- Matrix Normal Equations
#+END_CENTER
#+ATTR_HTML: :align center :cellpadding 1em
| <l>                       |         <r> |    <c>    |              <r> |    <c>    | <l>        |
| *Keystrokes* {{{SPC}}}    | *Key Codes* | {{{SPC}}} | *Stack Contents* | {{{SPC}}} | *Comments* |
| {{{c15( f LBL 5      )}}} |  =42,21, 5= |           |          ? ? ? ? |           |            |
| {{{c15( f RESULT E   )}}} |  =42,26,15= |           |          ? ? ? ? |           |            |
| {{{c15( RCL MATRIX A )}}} |  =45,16,11= |           |          A ? ? ? |           |            |
| {{{c15( RCL MATRIX B )}}} |  =45,16,12= |           |          B A ? ? |           |            |
| {{{c15( f MATRIX 5   )}}} |  =42,16, 5= |           |          B A ? ? |           |            |
| {{{c15( STO MATRIX B )}}} |  =44,16,12= |           |          E ? ? ? |           |            |
| {{{c15( RCL MATRIX A )}}} |  =45,16,11= |           |          A E ? ? |           |            |
| {{{c15( ENTER        )}}} |        =36= |           |          A A E ? |           |            |
| {{{c15( f MATRIX 5   )}}} |  =42,16, 5= |           |          E ? ? ? |           |            |
| {{{c15( STO MATRIX A )}}} |  =44,16,11= |           |          E ? ? ? |           |            |
| {{{c15( 0            )}}} |         =0= |           |          0 E ? ? |           |            |
| {{{c15( ENTER        )}}} |        =36= |           |          0 0 E ? |           |            |
| {{{c15( f DIM E      )}}} |  =42,23,15= |           |          0 0 E ? |           |            |
| {{{c15( g RTN        )}}} |     =43 32= |           |          ? ? ? ? |           |            |
...
The coefficients may be real or complex.  Complex mode will automatically be enabled if required.  This program consumes no storage registers or internal
labels.  It requires 31 program lines consuming 32 bytes (if I counted correctly).
...
- Exit Information
  - Exit Stack State:
    - {{{stk(T)}}}: Quadratic Discriminant
    - {{{stk(Z)}}}: Quadratic Discriminant
    - {{{stk(Y)}}}: Second Root
    - {{{stk(X)}}}: First Root
  - Exit Flag State:
...
| {{{c15( 4        )}}}  |         =4= |           |                    4 c b a |           |                       |
| {{{c15( ×        )}}}  |        =20= |           |                  4*c b a a |           |                       |
| {{{c15( g R↑     )}}}  |     =43 33= |           |                   a 4*cb a |           |                       |
| {{{c15( ÷        )}}}  |        =10= |           |                4*c/a b a a |           |                       |
| {{{c15( x≷y      )}}}  |        =34= |           |                b 4*c/a a a |           |                       |
| {{{c15( g R↑     )}}}  |     =43 33= |           |                a b 4*c/a a |           |                       |
| {{{c15( ÷        )}}}  |        =10= |           |              b/a 4*c/a a a |           |                       |
...
| {{{c15( x≷y      )}}}  |        =34= |           |      4*c/a D -b/a/2 -b/a/2 |           | Save off Discriminant |
| {{{c15( R↓       )}}}  |        =33= |           |      D -b/a/2 -b/a/2 4*c/a |           | Save off Discriminant |
| {{{c15( ENTER    )}}}  |        =36= |           |          D D -b/a/2 -b/a/2 |           | Save off Discriminant |
| {{{c15( g TEST 2 )}}}  |  =43,30, 2= |           |          D D -b/a/2 -b/a/2 |           |                       |
| {{{c15( g SF 8   )}}}  |  =43, 4, 8= |           |          D D -b/a/2 -b/a/2 |           | D<0 -> complex mode   |
| {{{c15( √x       )}}}  |        =11= |           |         √D D -b/a/2 -b/a/2 |           |                       |
| {{{c15( 2        )}}}  |         =2= |           |              2 √D D -b/a/2 |           |                       |
| {{{c15( ÷        )}}}  |        =10= |           |       √D/2 D -b/a/2 -b/a/2 |           |                       |
| {{{c15( g R↑     )}}}  |     =43 33= |           |       -b/a/2 √D/2 D -b/a/2 |           |                       |
| {{{c15( x≷y      )}}}  |        =34= |           |       √D/2 -b/a/2 D -b/a/2 |           |                       |
| {{{c15( -        )}}}  |        =30= |           |                R1 D -b/a/2 |           |                       |
| {{{c15( g LSTx   )}}}  |     =43 36= |           |           √D/2 R1 D -b/a/2 |           |                       |
| {{{c15( g R↑     )}}}  |     =43 33= |           |           -b/a/2 √D/2 R1 D |           |                       |
| {{{c15( +        )}}}  |        =40= |           |                  R2 R1 D D |           |                       |
| {{{c15( g RTN    )}}}  |     =43 32= |           |                  R2 R1 D D |           |                       |
...
|          {{{reg( .0 )}}}          | Temporary storage              | Whatever, but usually for programs      |
| {{{reg( .1 )}}} - {{{reg( .6 )}}} | Whatever                       | Whatever                                |
...
I normally keep =STO_CPLX=, =RCL_CPLX=, =NEWTON=, =STO_MAT=, =RCL_MAT=, =LIN_SLV=, =DET_A=, and =MAT_NML= in program memory.  This results in 156 lines of
program memory consumed by the programs.  I then add three zeros at the end as a little pad to separate my permanent programs from whatever I may add to the
...
| 143  | =42,21, 5= | {{{c15( f LBL 5 )}}} | [[#matNrm][Matrix Normal Equations]]   |
| 157  |        =0= | {{{c15( 0 )}}}       | Padding                                |
| 158  |        =0= | {{{c15( 0 )}}}       | Padding                                |
| 159  |        =0= | {{{c15( 0 )}}}       | Padding                                |
| 160  |        N/A | N/A                  | Solver programs etc...                 |
